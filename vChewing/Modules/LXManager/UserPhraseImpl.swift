...
extension UserPhraseInsertable {
  ...
  public var isDuplicated: Bool {
    LXMgr.checkIfPhrasePairExists(userPhrase: value, mode: inputMode, keyArray: keyArray)
  }
  ...
  public var isAlreadyFiltered: Bool {
    inputMode.lexicon.lxQuerier.isPairFiltered(pair: .init(keyArray: keyArray, value: value))
  }
  ...
  public func write(toFilter: Bool) -> Bool {
    LXAssembly.withFileHandleQueueSync {
      ...
      guard LXMgr.chkUserLMFilesExist(inputMode) else {
      ...
      /// 所以這裡不再強制要求整理格式。
      let theType: LXAssembly.ReplacableUserDataType = toFilter ? .theFilter : .thePhrases
      let theURL = LXMgr.userDictDataURL(mode: inputMode, type: theType)
      var fileSize: UInt64?
      do {
  ...
  public func removeFromFilter(
    ...
    LXAssembly.withFileHandleQueueSync {
      ...
      let theURL = LXMgr.userDictDataURL(mode: inputMode, type: .theFilter)
      if forceConsolidate,
         !LXAssembly.LXConsolidator.consolidate(path: theURL.path, pragma: false) { return false }
      // Get FileSize.
      var fileSize: UInt64?
      do {
        let dict = try FileManager.default.attributesOfItem(atPath: theURL.path)
      ...
      LXMgr.reloadUserFilterDirectly(mode: inputMode)
      return true
    }
  }
}
...
extension UserPhraseInsertable {
  ...
  public func suggestNextFreq(
    ...
    let fetchedUnigrams = inputMode.lexicon.lxQuerier.grams(for: keyArray)
