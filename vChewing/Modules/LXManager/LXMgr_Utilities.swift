...
extension LXMgr {
  ...
  public static func cassetteAccessFailureDescription(path: String? = nil) -> String {
    pathGuidanceDescription(baseKey: "i18n:LXMgr.accessFailure.cassette.description", path: path)
  }

  public static func cassettePathInvalidityDescription(path: String? = nil) -> String {
    pathGuidanceDescription(baseKey: "i18n:LXMgr.pathInvalidityFound.cassette.description", path: path)
  }

  public static func userDataFolderInvalidityDescription(path: String? = nil) -> String {
    pathGuidanceDescription(
      baseKey: "i18n:LXMgr.pathInvalidityFound.userDataFolder.description",
      path: path
    )
  }
  ...
  private static func pathGuidanceDescription(baseKey: String, path: String?) -> String {
    ...
    if iCloudMirroredPathSuspected(path) {
      resultStack.append("i18n:LXMgr.pathInvalidityFound.iCloudDriveManagedPathAdvice".i18n)
    }
    resultStack.append("i18n:LXMgr.pathInvalidityFound.suggestVerifyingSystemPrivacySettings".i18n)
  ...
  public static func migrateUserDataFrom(oldPath: String, to newPath: String) -> Int {
    var migratedCount = 0
    for type in LXAssembly.ReplacableUserDataType.allCases {
  ...
  private static func checkIfUserFilesExistBeforeOpening() -> Bool {
    if !Self.chkUserLMFilesExist(.imeModeCHS)
      ...
      let content = String(
        format: "i18n:ErrorMessage.CheckPermission:%@".i18n,
        Self.dataFolderPath(isDefaultFolder: false)
      )
      asyncOnMain {
        let alert = NSAlert()
        alert.messageText = "i18n:ErrorMessage.UnableToCreateUserPhraseFile".i18n
  ...
  private static func ensureFileExists(
    ...
    if !FileManager.default.fileExists(atPath: filePath) {
      ...
      if templateBasename != "" {
        ...
        do {
          try LXAssembly.withFileHandleQueueSync {
  ...
  public static func chkUserLMFilesExist(_ mode: Shared.InputMode) -> Bool {
    ...
    /// 後者的話，你在敲字時自己就會建立。
    var failed = false
    caseCheck: for type in LXAssembly.ReplacableUserDataType.allCases {
  ...
  internal static func templateName(
    for type: LXAssembly.ReplacableUserDataType,
    mode: Shared.InputMode
  )
    -> String {
    switch type {
...
extension LXMgr {
  ...
  public static func dumpUserDictDataToJSON(
    ...
    -> String? {
    var summarizedDict = [LXAssembly.UserDictionarySummarized]()
    Shared.InputMode.allCases.forEach { mode in
      guard mode != .imeModeNULL else { return }
      summarizedDict.append(mode.lexicon.summarize(all: all))
    }
