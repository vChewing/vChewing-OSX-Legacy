...
// MARK: - LXAssembly.PerceptionPersistor
...
extension LXAssembly.PerceptionPersistor {
  ...
  private struct JournalRecord: Codable {
    ...
    init(
      ...
      key: String? = nil,
      pair: LXAssembly.LXPerceptor.KeyPerceptionPair? = nil
    ) {
    ...
    var key: String?
    var pair: LXAssembly.LXPerceptor.KeyPerceptionPair?
  }
}
      ...
      do {
        ...
        if !emptyContents.contains(trimmed) {
          let jsonResult = try decoder.decode(
            [LXAssembly.LXPerceptor.KeyPerceptionPair].self, from: data
          )
...
extension LXAssembly.PerceptionPersistor {
  ...
  nonisolated private func replayJournal(
    ...
    do {
      ...
      var tempMap = [String: LXAssembly.LXPerceptor.KeyPerceptionPair]()
      var mutated = false
