...
// MARK: - LXAssembly.LXCoreEX

extension LXAssembly {
  /// 與之前的 LXCore 不同，LXCoreEX 不在辭典內記錄實體，而是記錄位元組範圍。
  ...
  struct LXCoreEX {
    ...
    mutating func open(_ path: String) -> Bool {
      ...
      do {
        ...
        let newBytes: [UInt8] = try LXAssembly.withFileHandleQueueSync {
          if consolidated {
