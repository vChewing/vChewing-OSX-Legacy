...
public protocol InputHandlerProtocol: AnyObject {
  ...
  var currentLM: LXAssembly.LXFacade { get set }
...
extension InputHandlerProtocol {
  ...
  func generateArrayOfAssociates(withPairs pairs: [Homa.CandidatePair]) -> [(
    ...
    currentLM.lxQuerier.associatedCandidates(forPairs: pairs)
  }
  ...
  func generateArrayOfAssociates(withPair pair: Homa.CandidatePair) -> [(
    ...
    currentLM.lxQuerier.associatedCandidates(forPair: pair)
  }
  ...
  func generateArrayOfCandidates(fixOrder: Bool = true) -> [CandidateInState] {
    ...
    if currentTypingMethod == .vChewingFactory,
      ...
      eTenSequenceEnforcement: if prefs.enforceETenDOSCandidateSequence || prefs.useSCPCTypingMode {
        let seq4ETen = currentLM.lxQuerier.supplementalValues(
  ...
  private func supplementalETenSingleKanjiCandidates(
    ...
    return currentLM.lxQuerier.supplementalValues(
      for: reading,
      strategy: .configuredLookup
  ...
  func filterPOMAppendables(
    from suggestion: LXAssembly.OverrideSuggestion,
...
extension InputHandlerProtocol {
  /// Homa 已內建上下文鞏固邏輯；OSNeutralAssembly 僅負責將當前游標風格轉譯給 Homa。
