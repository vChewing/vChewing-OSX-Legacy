...
extension LXAssembly.CassetteSortedMap {
  ...
  static func build(from dictionary: [String: [String]]) -> Self {
    ...
    var entries = [LXAssembly.CassetteMapEntry]()
...
extension LXAssembly.CassetteReverseIndex {
  ...
  static func build(
    charDefMap: LXAssembly.CassetteSortedMap,
    symbolDefMap: LXAssembly.CassetteSortedMap
  )
    -> Self {
    ...
    var revChars = [UInt8]()
    var revEntries = [LXAssembly.CassetteReverseEntry]()
...
extension LXAssembly.CassetteQuickMap {
  static func build(from dictionary: [String: String]) -> Self {
    ...
    var rawData = [UInt8]()
    rawData.reserveCapacity(totalBytes)
    var entries = [LXAssembly.CassetteQuickEntry]()
    entries.reserveCapacity(sortedKeys.count)
    for key in sortedKeys {
      guard let value = dictionary[key] else { continue }
      let keyStart = UInt32(rawData.count)
...
extension LXAssembly.CassetteOctagramMap {
  static func build(from dictionary: [String: Int]) -> Self {
    ...
    var rawData = [UInt8]()
    rawData.reserveCapacity(totalBytes)
    var entries = [LXAssembly.CassetteOctagramEntry]()
    entries.reserveCapacity(sortedKeys.count)
    for key in sortedKeys {
...
extension LXAssembly.CassetteOctagramDividedMap {
  static func build(from dictionary: [String: (Int, String)]) -> Self {
    ...
    var rawData = [UInt8]()
    rawData.reserveCapacity(totalBytes)
    var entries = [LXAssembly.CassetteOctagramDividedEntry]()
    entries.reserveCapacity(sortedKeys.count)
    for key in sortedKeys {
      guard let value = dictionary[key] else { continue }
      let keyStart = UInt32(rawData.count)
    ...
    if FileManager.default.fileExists(atPath: path) {
      do {
        guard let fileHandle = FileHandle(forReadingAtPath: path) else {
          throw LXAssembly.FileErrors.fileHandleError("")
        }
